...
namespace MToolKit.Runtime.Persistence.ES3Integration
{
    ...
    public class ProfileManager : IProfileManager, IDisposable
    {
        ...
        public ProfileManager(IES3Service es3Service, ES3SaveConfig config)
        {
            ...
            this.profilesDirectory = Path.Combine(config.GetSaveDirectoryPath(), "Profiles");
        ...
        private void OnCurrentProfileChanged(string profileName)
        {
            ...
            try
            {
                ...
                {
                    var metadata = GetProfileMetaData(profileName);
        ...
        public (bool success, string actualProfileName) CreateProfileWithName(string profileName, CancellationToken ct = default)
        {
            ...
            try
            {
                var filePath = GetProfileFilePath(actualProfileName);
                ProfileMetaData metadata = new ProfileMetaData(actualProfileName, "Never", config.SaveFormatVersion, 0);

                // Create the profile file with initial metadata using ES3Settings
                var es3Settings = new ES3Settings(filePath)
                {
                ...
                var verification = ES3.Load<ProfileMetaData>("ProfileMetadata", es3Settings);
        ...
        {
            var (success, _) = CreateProfileWithName(profileName, ct);
            return success;
        }
        ...
        public UniTask<bool> LoadProfileAsync(string profileName, CancellationToken ct = default)
        {
            ...
            try
            {
                ...
                var profileFilePath = GetProfileFilePath(profileName);
                ...
                var profileSettings = new ES3Settings(profileFilePath)
                {
                    compressionType = config.CompressSaveData ? ES3.CompressionType.Gzip : ES3.CompressionType.None,
                    encryptionType = config.EncryptSaveData ? ES3.EncryptionType.AES : ES3.EncryptionType.None,
                    encryptionPassword = config.EncryptionKey
        ...
        public bool SaveProfile(string profileName, CancellationToken ct = default)
        {
            ...
            try
            {
                ...
                var existingMetadata = GetProfileMetaData(profileName, ct);
                if (existingMetadata != null)
                {
                    var updatedMetadata = new ProfileMetaData(
        ...
        public ProfileMetaData GetProfileMetaData(string profileName, CancellationToken ct = default)
        {
            ...
            try
            {
                ...
                {
                    var metadata = ES3.Load<ProfileMetaData>("ProfileMetadata", es3Settings);
                    ...
                    else
                    {
                        ...
                        var profileNameVal = ES3.KeyExists("ProfileName", es3Settings) ? ES3.Load<string>("ProfileName", es3Settings) : profileName;
                        var lastSaveTimeVal = ES3.KeyExists("LastSaveTime", es3Settings) ? ES3.Load<string>("LastSaveTime", es3Settings) : "Never";
                        var saveFormatVersionVal = ES3.KeyExists("SaveFormatVersion", es3Settings) ? ES3.Load<string>("SaveFormatVersion", es3Settings) : config.SaveFormatVersion;
                        var saveCounterVal = ES3.KeyExists("SaveCounter", es3Settings) ? ES3.Load<int>("SaveCounter", es3Settings) : 0;
                        
                        var fixedMetadata = new ProfileMetaData(profileNameVal, lastSaveTimeVal, saveFormatVersionVal, saveCounterVal);
                ...
                var profileNameValue = ES3.KeyExists("ProfileName", es3Settings) ? ES3.Load<string>("ProfileName", es3Settings) : profileName;
                var lastSaveTimeValue = ES3.KeyExists("LastSaveTime", es3Settings) ? ES3.Load<string>("LastSaveTime", es3Settings) : "Never";
                var saveFormatVersionValue = ES3.KeyExists("SaveFormatVersion", es3Settings) ? ES3.Load<string>("SaveFormatVersion", es3Settings) : config.SaveFormatVersion;
                var saveCounterValue = ES3.KeyExists("SaveCounter", es3Settings) ? ES3.Load<int>("SaveCounter", es3Settings) : 0;
                
                var fallbackMetadata = new ProfileMetaData(profileNameValue, lastSaveTimeValue, saveFormatVersionValue, saveCounterValue);
                ...
                if (string.IsNullOrEmpty(fallbackMetadata.LastSaveTime) || fallbackMetadata.LastSaveTime == "Never")
                {
                    ...
                    if (fileInfo.Exists)
                    {
                        ...
                        fallbackMetadata = new ProfileMetaData(profileName, fileInfo.LastWriteTime.ToString("yyyy-MM-dd HH:mm:ss"), config.SaveFormatVersion, 0);
                    }
                }
                
                log.ForMethod().Debug("GetProfileMetaData: Created fallback metadata for '{0}' - ProfileName='{1}', LastSaveTime='{2}', SaveFormatVersion='{3}', SaveCounter={4} (read-only)", 
        ...
        public List<ProfileMetaData> GetAllProfileMetadata(CancellationToken ct = default)
        {
            ...
            var availableProfiles = AvailableProfiles.Value;
        ...
        public UniTask<string> GetMostRecentProfileAsync(CancellationToken ct = default)
        {
            ...
            try
            {
                ...
                var orderedProfiles = allMetaData.OrderByDescending(m => 
                {
                    if (m.LastSaveTime != null && DateTime.TryParse(m.LastSaveTime, out DateTime lastSaveTime))
                        return lastSaveTime;
                    return DateTime.MinValue;
                ...
                try
                {
                    ...
                    if (ES3.KeyExists("ProfileMetadata", es3Settings))
                    {
                        ...
                        if (metadata == null || string.IsNullOrEmpty(metadata.ProfileName))
                        {
                            ...
                            var repairedMetadata = new ProfileMetaData(
        ...
        public void RepairAllProfileMetadata()
        {
            ...
            foreach (string profileName in availableProfiles ?? new List<string>())
            {
                try
                {
                    ...
                    var repairedMetadata = RepairProfileMetadata(profileName, es3Settings);
        ...
        private ProfileMetaData RepairProfileMetadata(string profileName, ES3Settings es3Settings)
        {
            try
            {
                ...
                var filePath = GetProfileFilePath(profileName);
                var properES3Settings = new ES3Settings(filePath)
                {
                    compressionType = config.CompressSaveData ? ES3.CompressionType.Gzip : ES3.CompressionType.None,
                    encryptionType = config.EncryptSaveData ? ES3.EncryptionType.AES : ES3.EncryptionType.None,
                    encryptionPassword = config.EncryptionKey
                ...
                catch (Exception ex)
                {
                    ...
                    try
                    {
                        var json = JsonUtility.ToJson(repairedMetadata);
                ...
                var verificationMetadata = ES3.Load<ProfileMetaData>("ProfileMetadata", properES3Settings);
        ...
        private void RefreshAvailableProfiles()
        {
            try
            {
                ...
                var profileFiles = Directory.GetFiles(profilesDirectory, "*.es3")
