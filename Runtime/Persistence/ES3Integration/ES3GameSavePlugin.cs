...
namespace MToolKit.Runtime.Persistence.ES3Integration
{
    ...
    {
        [Inject] private SaveSystemCoordinator saveSystemCoordinator;
        [Inject] private IES3Service es3Service;

        [SerializeField] [Required] private GameObject autoSaveExecutingAnimatedObject;

        private List<IDisposable> disposables = new();

        [ShowInInspector] [ReadOnly] private ES3GameSaveSystem Service => GetService();
        ...
        {
            log.ForGameObject(gameObject).ForMethod(nameof(CreateService)).Debug("Creating ES3GameSaveSystem with config");
            ...
            var resolvedEs3Service = resolver.Resolve<IES3Service>();
        ...
        public async UniTask StartAsync(CancellationToken cancellation)
        {
            ...
            {
                log.ForGameObject(gameObject).ForMethod(nameof(StartAsync)).Verbose(
                    "ES3GameSavePlugin already runtime initialized, skipping");
                return;
            }

            log.ForGameObject(gameObject).ForMethod(nameof(StartAsync)).Debug(
            ...
            try
            {
                ...
                if (service == null)
                {
                    log.ForGameObject(gameObject).ForMethod(nameof(StartAsync)).Error(
                        "Failed to resolve ES3GameSaveSystem");
                    return;
                }

                // Use the injected SaveSystemCoordinator
                var resolvedSaveSystemCoordinator = saveSystemCoordinator;

                log.ForGameObject(gameObject).ForMethod(nameof(StartAsync)).Information(
                ...
                resolvedSaveSystemCoordinator.IsAutoSaveExecuting.Subscribe(isAutoSaveExecuting =>
                {
                    ...
                    {
                        log.ForGameObject(gameObject).ForMethod(nameof(StartAsync)).Warning(
                ...
                var currentValue = resolvedSaveSystemCoordinator.IsAutoSaveExecuting.Value;
                if (autoSaveExecutingAnimatedObject != null)
                {
            ...
            {
                log.ForGameObject(gameObject).ForMethod(nameof(StartAsync)).Error(ex,
        ...
        private ES3GameSaveSystem GetService()
        {
            ...
            try
            {
                ...
                if (globalInstaller != null)
                {
                    ...
                    var resolvedService = resolver.Resolve<IES3GameSaveSystem>() as ES3GameSaveSystem;
