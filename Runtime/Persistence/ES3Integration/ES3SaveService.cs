...
namespace MToolKit.Runtime.Persistence.ES3Integration
{
    ...
    private static ILogger log => logLazy.Value ?? Serilog.Core.Logger.None;
        ...
        public ES3SaveService(string filePath = "SaveFile.es3")
        {
            ...
            this.saveFormatVersion = "1.0.0"; // Default version for backward compatibility
            this.disposalCts = new CancellationTokenSource();
            this.fileAccessSemaphore = new SemaphoreSlim(1, 1); // Allow only one concurrent file access
            
            // Create ES3Settings directly
            this.es3Settings = new ES3Settings(filePath);
        ...
        {
            this.filePath = filePath;
            this.saveFormatVersion = config?.SaveFormatVersion ?? "1.0.0";
            this.config = config;
            this.disposalCts = new CancellationTokenSource();
            this.fileAccessSemaphore = new SemaphoreSlim(1, 1); // Allow only one concurrent file access
