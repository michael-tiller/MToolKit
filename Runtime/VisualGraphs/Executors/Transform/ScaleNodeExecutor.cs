...
{
  /// <summary>
  ///   Executor for ScaleNode. Reads a Vector3 state key (missing/wrong-type → Vector3.zero + warning),
  ///   writes it to another state key.
  /// </summary>
  public sealed class ScaleNodeExecutor : IGraphNodeExecutor
  {
    private static readonly Lazy<ILogger> logLazy = new(() =>
      Log.Logger.ForContext<ScaleNodeExecutor>().ForFeature("VisualGraphs.Transform"));
    private static ILogger log => logLazy.Value ?? Logger.None;

    public string NodeType => "ScaleNode";
