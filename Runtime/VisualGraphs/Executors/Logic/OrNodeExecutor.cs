...
{
  /// <summary>
  ///   Executor for OrNode. Folds a list of bool state keys with OR, left-to-right (empty list → false;
  ///   missing/unresolvable keys are skipped from the fold), writes the result.
  /// </summary>
  public sealed class OrNodeExecutor : IGraphNodeExecutor
  {
    private static readonly Lazy<ILogger> logLazy = new(() =>
      Log.Logger.ForContext<OrNodeExecutor>().ForFeature("VisualGraphs.Logic"));
    private static ILogger log => logLazy.Value ?? Logger.None;

    public string NodeType => "OrNode";
      ...
      if (keys != null)
        foreach (var key in keys)
        {
          if (!string.IsNullOrEmpty(key) && state.TryGet<bool>(key, out var value))
            ...
            log.ForMethod().Warning("Logic: state key '{Key}' missing or not a bool, skipped from OR fold", key);
        }
