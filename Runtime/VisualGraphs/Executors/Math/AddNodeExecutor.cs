...
{
  /// <summary>
  ///   Executor for AddNode. Reads two float state keys, writes their sum to a third.
  /// </summary>
  public sealed class AddNodeExecutor : IGraphNodeExecutor
  {
    private static readonly Lazy<ILogger> logLazy = new(() =>
      Log.Logger.ForContext<AddNodeExecutor>().ForFeature("VisualGraphs.Math"));
    private static ILogger log => logLazy.Value ?? Logger.None;

    public string NodeType => "AddNode";
