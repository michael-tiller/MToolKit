...
{
  /// <summary>
  ///   Executor for MultiplyNode. Reads two float state keys, writes their product to a third.
  /// </summary>
  public sealed class MultiplyNodeExecutor : IGraphNodeExecutor
  {
    private static readonly Lazy<ILogger> logLazy = new(() =>
      Log.Logger.ForContext<MultiplyNodeExecutor>().ForFeature("VisualGraphs.Math"));
    private static ILogger log => logLazy.Value ?? Logger.None;

    public string NodeType => "MultiplyNode";
      ...
      CancellationToken ct = default)
    {
      ...
      var result = ReadFloat(aKey, state) * ReadFloat(bKey, state);
