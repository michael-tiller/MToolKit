...
{
  /// <summary>
  ///   Executor for RequestClaimQuestNode.
  ///   Loads a quest definition and publishes ClaimQuestRequestMessage via GameMessageBroker.
  /// </summary>
  public sealed class RequestClaimQuestNodeExecutor : IGraphNodeExecutor
  {
    private static readonly Lazy<ILogger> logLazy = new(() =>
      Log.Logger.ForContext<RequestClaimQuestNodeExecutor>().ForFeature("VisualGraphs.Quest"));
    private static ILogger log => logLazy.Value ?? Logger.None;

    public string NodeType => "RequestClaimQuestNode";
      ...
      CancellationToken ct = default)
    {
      ...
      {
        log.ForMethod().Warning("Quest: RequestClaimQuestNode has no 'Quest' parameter, continuing execution");
      ...
      if (questParam is QuestAssetReference questAssetRef)
      {
        ...
        {
          log.ForMethod().Warning("Quest: RequestClaimQuestNode QuestAssetReference has no GUID, continuing execution");
          ContinueToNextNodes(graph, node, context);
          return;
        }
      ...
      else if (questParam is SerializableAssetReference assetRef)
      {
        ...
        {
          log.ForMethod().Warning("Quest: RequestClaimQuestNode SerializableAssetReference has no GUID, continuing execution");
          ContinueToNextNodes(graph, node, context);
          return;
        }
      ...
      {
        log.ForMethod().Warning("Quest: RequestClaimQuestNode 'Quest' parameter is not a QuestAssetReference or SerializableAssetReference (type: {Type}), continuing execution",
      ...
      if (questDef == null)
      {
        log.ForMethod().Warning("Quest: RequestClaimQuestNode has null quest definition, continuing execution");
        ContinueToNextNodes(graph, node, context);
        return;
      }

      // Publish ClaimQuestRequestMessage via GameMessageBroker
      log.ForMethod().Information("Quest: Publishing ClaimQuestRequestMessage for quest: {QuestName} ({QuestGuid})", questDef.DisplayName, questDef.Guid);

      try
      {
        GameMessageBroker.Publish(new ClaimQuestRequestMessage(questDef.Guid));
        log.ForMethod().Information("Quest: Successfully published ClaimQuestRequestMessage for quest: {QuestName} ({QuestGuid})", questDef.DisplayName, questDef.Guid);
      }
      catch (Exception ex)
      {
        log.ForMethod().Error(ex, "Quest: Error publishing ClaimQuestRequestMessage for quest: {QuestName} ({QuestGuid}), {Message}", questDef.DisplayName, questDef.Guid, ex.Message);
      }
