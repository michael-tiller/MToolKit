...
namespace MToolKit.Runtime.VisualGraphs.Installer
{
    ...
    public sealed class VisualGraphInstaller : IInstaller
    {
        public void Install(IContainerBuilder builder)
        {
            ...
            builder.Register<Executors.QuestSetStageNodeExecutor>(Lifetime.Singleton)
                .As<IGraphNodeExecutor>();
            builder.Register<Executors.DialogueLineNodeExecutor>(Lifetime.Singleton)
                .As<IGraphNodeExecutor>();
            builder.Register<Executors.DialogueChoiceNodeExecutor>(Lifetime.Singleton)
                .As<IGraphNodeExecutor>();
            ...
            builder.RegisterBuildCallback(container =>
            {
                ...
                var executors = container.Resolve<System.Collections.Generic.IEnumerable<IGraphNodeExecutor>>();
    ...
    internal sealed class SimpleEventEmitter : IEventEmitter
    {
        public void Emit(IEventMessage message)
        {
            ...
            // For now, just log
            UnityEngine.Debug.Log($"[GraphEvent] {message.Type} in {message.Domain}");
        }
    }
}
