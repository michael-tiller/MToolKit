...
namespace MToolKit.Runtime.Navigation.Services
{
    ...
    public class NavigationService : INavigationService
    {
            ...
            where T : View
        {
            ...
            T foundView = null;
            var allInStack = stacks[canvasType].ToArray(); // top is last in array
            ...
            if (foundView != null)
            {
                Stack<IView> tempStack = new Stack<IView>();

                // Pop until we get to our found item
                while (stacks[canvasType].Peek() != foundView)
                {
        ...
        public async UniTask PopAsync(ECanvasType canvasType, CancellationToken token = default)
        {
            ...
            var currentView = stacks[canvasType].Peek();
            ...
            if (stacks[canvasType].Count > 0)
            {
                var nextView = stacks[canvasType].Peek();
