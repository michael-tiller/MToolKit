...
namespace MToolKit.Runtime.Components
{
  public class KeybindListElement : MonoBehaviour
  {
    ...
    private static List<KeybindListElement> keybindListElements;
    ...
    private static void OnActionChange(object obj, InputActionChange change)
    {
      ...
      foreach (KeybindListElement component in keybindListElements)
      {
        InputAction referencedAction = component.InputActionReference?.action;
    ...
    {
      keybindListElements ??= new List<KeybindListElement>();
      keybindListElements.Add(this);
      if (keybindListElements.Count == 1)
        InputSystem.onActionChange += OnActionChange;
    }

    protected void OnDisable()
    {
      keybindListElements.Remove(this);
      if (keybindListElements.Count != 0) return;
      keybindListElements = null;
      InputSystem.onActionChange -= OnActionChange;
    }
    ...
    private void UpdateBindingDisplay(int bindingIndex, TextMeshProUGUI textComponent)
    {
      ...
      string displayString = action.GetBindingDisplayString(bindingIndex, DisplayStringOptions);
    ...
    private void UpdateBindingDisplayWithIcons(int bindingIndex, TextMeshProUGUI textComponent, Image iconComponent)
    {
      ...
      string deviceLayoutName = binding.effectivePath?.Split('/')[0];
      string controlPath = binding.effectivePath?.Split('/').LastOrDefault();
    ...
    {
      if (!ResolveAction(out InputAction action))
      {
    ...
    public void ResetSlotToDefault(int slotIndex)
    {
      ...
      if (action.bindings[slotIndex].isComposite)
        // It's a composite. Remove overrides from part bindings.
        for (int i = slotIndex + 1; i < action.bindings.Count && action.bindings[i].isPartOfComposite; ++i)
          action.RemoveBindingOverride(i);
      else
    ...
    private void ShowRebindingState(bool isRebinding, int bindingIndex = -1, bool hasConflict = false)
    {
      if (isRebinding)
      {
        ...
        if (TextRebindingHint != null)
        {
          ...
          InputAction action = InputActionReference?.action;
          string slotName = bindingIndex switch
          {
            0 => "Primary",
            1 => "Secondary", 
            2 => "Gamepad",
          ...
          TextRebindingHint.text =
            $"{partName}{LocalizationHelper.GetLocalizedString("Press any key to rebind")} {LocalizationHelper.GetLocalizedString(slotName)} {LocalizationHelper.GetLocalizedString("slot")}...";
        }
