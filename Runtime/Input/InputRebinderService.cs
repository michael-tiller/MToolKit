...
namespace MToolKit.Runtime.Input
{
    ...
    public class InputRebinderService : IDisposable, IInputRebinderService
    {
        ...
        public void RegisterAction(InputAction action)
        {
            if (action == null)
            {
                log.ForMethod(nameof(RegisterAction)).Error("Cannot register null input action");
                return;
            }
            ...
            {
                log.ForMethod(nameof(RegisterAction)).Warning("Action {ActionName} is already registered", action.name);
                return;
            }
            ...
            log.ForMethod(nameof(RegisterAction)).Information("Registered action {ActionName} for rebinding", action.name);
        }
        ...
        public bool StartInteractiveRebinding(InputAction action, int bindingIndex, string[] excludeControls = null)
        {
            if (action == null)
            {
                log.ForMethod(nameof(StartInteractiveRebinding)).Error("Cannot start interactive rebinding: action is null");
                return false;
            }

            if (bindingIndex < 0 || bindingIndex >= action.bindings.Count)
            {
                log.ForMethod(nameof(StartInteractiveRebinding)).Error("Invalid binding index {BindingIndex} for action {ActionName}", bindingIndex, action.name);
                return false;
            }

            if (isRebinding)
            {
                log.ForMethod(nameof(StartInteractiveRebinding)).Warning("Cannot start rebinding: another rebinding operation is already in progress");
                return false;
            }
            ...
            {
                // For composite bindings, we need to rebind each part in sequence
        ...
        private bool StartSingleBindingRebinding(InputAction action, int bindingIndex, string[] excludeControls)
        {
            ...
            currentRebindingOperation = action.PerformInteractiveRebinding(bindingIndex)
                .OnCancel(operation =>
                {
                    log.ForMethod(nameof(StartSingleBindingRebinding)).Information("Rebinding cancelled for {ActionName} binding {BindingIndex}", action.name, bindingIndex);
                    OnRebindingCompleted.OnNext((action, bindingIndex, false));
                    CleanupRebindingOperation();
                })
                .OnComplete(operation =>
                {
                    var newPath = operation.action.bindings[bindingIndex].effectivePath;
                    
                    // Check for conflicts
                    if (CheckBindingConflict(action, bindingIndex, newPath))
                    {
                        log.ForMethod(nameof(StartSingleBindingRebinding)).Warning("Binding conflict detected for {ActionName} binding {BindingIndex} with path {NewPath}", 
                    ...
                    {
                        log.ForMethod(nameof(StartSingleBindingRebinding)).Information("Rebinding completed for {ActionName} binding {BindingIndex} to {NewPath}", 
                            action.name, bindingIndex, newPath);
            ...
            {
                foreach (var control in excludeControls)
                {
            ...
            log.ForMethod(nameof(StartSingleBindingRebinding)).Information("Started interactive rebinding for {ActionName} binding {BindingIndex}", 
        ...
        private bool StartCompositeRebinding(InputAction action, int bindingIndex, string[] excludeControls)
        {
            ...
            {
                log.ForMethod(nameof(StartCompositeRebinding)).Error("Invalid composite binding structure for {ActionName} binding {BindingIndex}", action.name, bindingIndex);
                return false;
            }
        ...
        private bool StartCompositePartRebinding(InputAction action, int partIndex, string[] excludeControls)
        {
            ...
            currentRebindingOperation = action.PerformInteractiveRebinding(partIndex)
                .OnCancel(operation =>
                {
                    log.ForMethod(nameof(StartCompositePartRebinding)).Information("Composite rebinding cancelled for {ActionName} part {PartIndex}", action.name, partIndex);
                    OnRebindingCompleted.OnNext((action, partIndex, false));
                    CleanupRebindingOperation();
                })
                .OnComplete(operation =>
                {
                    var newPath = operation.action.bindings[partIndex].effectivePath;
                    
                    // Check for conflicts
                    if (CheckBindingConflict(action, partIndex, newPath))
                    {
                        log.ForMethod(nameof(StartCompositePartRebinding)).Warning("Binding conflict detected for {ActionName} part {PartIndex} with path {NewPath}", 
                    ...
                    log.ForMethod(nameof(StartCompositePartRebinding)).Information("Composite part rebinding completed for {ActionName} part {PartIndex} to {NewPath}", 
                        action.name, partIndex, newPath);
            ...
            log.ForMethod(nameof(StartCompositePartRebinding)).Information("Started composite part rebinding for {ActionName} part {PartIndex}", 
        ...
        public void RemoveBindingOverride(InputAction action, int bindingIndex)
        {
            if (action == null)
            {
                log.ForMethod(nameof(RemoveBindingOverride)).Error("Cannot remove binding override: action is null");
                return;
            }

            if (bindingIndex < 0 || bindingIndex >= action.bindings.Count)
            {
                log.ForMethod(nameof(RemoveBindingOverride)).Error("Invalid binding index {BindingIndex} for action {ActionName}", bindingIndex, action.name);
                return;
            }
            ...
            log.ForMethod(nameof(RemoveBindingOverride)).Information("Removed binding override for {ActionName} binding {BindingIndex}", 
                action.name, bindingIndex);
        }
        ...
        public void ResetActionBindings(InputAction action)
        {
            if (action == null)
            {
                log.ForMethod(nameof(ResetActionBindings)).Error("Cannot reset bindings: action is null");
                return;
            }
            ...
            log.ForMethod(nameof(ResetActionBindings)).Information("Reset bindings for action {ActionName}", action.name);
        }
        ...
        public void ResetAllBindings()
        {
            ...
            log.ForMethod(nameof(ResetAllBindings)).Information("Reset all action bindings");
        }
        ...
        public bool CheckBindingConflict(InputAction action, int bindingIndex, string newBindingPath)
        {
            ...
            foreach (var registeredAction in registeredActions.Values)
            {
                ...
                {
                    var binding = registeredAction.bindings[i];
                    if (binding.effectivePath == newBindingPath)
                    {
                        var conflictMessage = $"Binding conflict: {newBindingPath} is already used by {registeredAction.name}";
        ...
        public bool ResolveBindingConflict(InputAction conflictingAction, int conflictingBindingIndex)
        {
            ...
            {
                log.ForMethod(nameof(ResolveBindingConflict)).Error("Invalid parameters for conflict resolution");
                return false;
            }

            var bindingPath = conflictingAction.bindings[conflictingBindingIndex].effectivePath;
            conflictingAction.RemoveBindingOverride(conflictingBindingIndex);
            
            log.ForMethod(nameof(ResolveBindingConflict)).Information("Resolved binding conflict by removing {BindingPath} from {ActionName} binding {BindingIndex}", 
        ...
        {
            var result = new List<(InputAction action, int bindingIndex)>();
