...
namespace MToolKit.Runtime.Editor
{
    ...
    public class PluginDiagnosisWindow : EditorWindow
  {
    ...
    private int selectedInstallerTab;
    ...
    private static bool CheckInstallerExists(string installerName)
    {
      try
      {
        ...
        Type installerType = PluginDiagnosticService.FindTypeByName(installerName);
        if (installerType != null)
        {
          PropertyInfo instanceProperty = installerType.GetProperty("Instance", BindingFlags.Public | BindingFlags.Static);
          if (instanceProperty != null)
          {
            object installer = instanceProperty.GetValue(null);
            if (installer != null) return true;
          }
        }

        // Try scene search as fallback
        Object[] installerObjects = FindObjectsByType(typeof(MonoBehaviour), FindObjectsSortMode.None)
    ...
    private void DrawControls()
    {
      ...
      EditorGUILayout.BeginVertical(EditorStyles.helpBox);
      {
        ...
        int newTab = GUILayout.Toolbar(selectedInstallerTab, installerTabs, GUILayout.Height(25));
    ...
    {
      List<PluginStateModel> filteredPlugins = GetFilteredPlugins();
      ...
      EditorGUILayout.BeginVertical();
      {
        ...
        if (showDebugInfo)
        {
          EditorGUILayout.BeginVertical(EditorStyles.helpBox);
          {
            ...
            bool globalInstallerExists = CheckInstallerExists("GlobalInstaller");
            bool gameInstallerExists = CheckInstallerExists("GameInstaller");
        ...
        scrollPosition = EditorGUILayout.BeginScrollView(scrollPosition);
        {
          if (filteredPlugins.Count == 0)
            EditorGUILayout.HelpBox($"No plugins found for {installerTabs[selectedInstallerTab]}. Make sure the game is running and the installer is present in the scene.",
              MessageType.Warning);
          else
            foreach (PluginStateModel pluginState in filteredPlugins)
    ...
    private void DrawPluginState(PluginStateModel pluginState)
    {
      ...
      EditorGUILayout.BeginVertical(EditorStyles.helpBox);
      {
        ...
        EditorGUILayout.BeginHorizontal();
        {
          Color statusColor = GetStatusColor(pluginState.Status);
        ...
        if (showDependencyDetails && pluginState.Dependencies.Any())
        {
          ...
          foreach (DependencyModel dep in pluginState.Dependencies)
          {
            Color depColor = dep.IsResolved ? Color.green : Color.red;
            Color originalColor = GUI.color;
            GUI.color = depColor;

            string depText = dep.IsOptional ? $"{dep.ServiceName} (Optional)" : dep.ServiceName;
            string statusIcon = dep.IsResolved ? "✓" : "✗";
    ...
    {
      IEnumerable<PluginStateModel> filtered = pluginStates.AsEnumerable();
    ...
    private void RefreshPluginStates()
    {
      try
      {
        string installerType = installerTabs[selectedInstallerTab];
